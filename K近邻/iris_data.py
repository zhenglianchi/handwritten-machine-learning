...
def KNN(K,x_train,y_train,x_test,y_test):
    ...
    for item_test in x_test:
        ...
        if list(nn_y).count(0)>list(nn_y).count(1) and list(nn_y).count(0)>list(nn_y).count(2):
            predict.append(0)
        elif list(nn_y).count(1)>list(nn_y).count(0) and list(nn_y).count(1)>list(nn_y).count(2):
            predict.append(1)
        else:
            predict.append(2)
    return predict
...
predict=KNN(3,x_train,y_train,x_test,y_test)
correct_rate=prediction(predict,y_test)
print("鸢尾花分类准确率为：%.2f"%correct_rate)